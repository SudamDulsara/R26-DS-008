...
def calculate_cer(hypothesis: str, reference: str) -> float:
    ...
    Both sides are normalised first, so the score reflects OCR errors rather
    than differences in how the text happened to be encoded. See
    pipeline/normalize.py for why that distinction matters in Sinhala.
    """
...
def calculate_wer(hypothesis: str, reference: str) -> float:
    ...
    Both sides are normalised first — see calculate_cer().
    """
    hypothesis, reference = normalize(hypothesis), normalize(reference)
    if not reference.strip():
        return 0.0
